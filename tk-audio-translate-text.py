...
def transcribe_files():
    ...
    with open(save_path, "a") as f, open(save_path[:-4] + "_translated.txt", "a") as f_translated:
        for file in mp3_files:
            audio_file = open(file, "rb")
            transcript = openai.Audio.transcribe("whisper-1", audio_file)
            translated_transcript = translate_text(transcript.text)
            f.write(transcript.text + "\n")
            f_translated.write(translated_transcript + "\n")

    # Open the text files
    os.startfile(save_path)
    os.startfile(save_path[:-4] + "_translated.txt")
